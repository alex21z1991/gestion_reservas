...
class ReservaForm(forms.ModelForm):
    """
    Formulario de creación de reserva. Replica las validaciones que ya
    tenían en JS dentro de reservas.html (teléfono chileno de 9 dígitos,
    fecha no pasada y máximo 90 días a futuro).
    """
    ...
    class Meta:
        ...
        widgets = {
            ...
            "comensales": forms.Select(attrs={"class": "form-select custom-input"}),
            "ocasion": forms.Select(attrs={"class": "form-select custom-input"}),
    ...
    def __init__(self, *args, horas_disponibles=None, **kwargs):
        ...
        horas_disponibles = horas_disponibles or ["13:00", "14:00", "15:00", "16:00", "17:00"]
        self.fields["hora"].choices = [(h, h) for h in horas_disponibles]
        self.fields["comensales"].widget.choices = [
            (i, f"{i} persona" if i == 1 else f"{i} personas") for i in range(1, 8)
        ] + [(8, "8 o más personas")]
